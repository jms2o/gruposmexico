...
interface MoreGroupsSectionProps {
  groups: any[];
  whatsappNum: string;
  cityName: string;
}

const MoreGroupsSection = ({ groups, whatsappNum, cityName }: MoreGroupsSectionProps) => {
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl sm:text-2xl font-display font-bold text-foreground">
            Más grupos en <span className="text-gold">{cityName}</span>
          </h2>
        </div>
      ...
      >
        {groups.map((group: any, idx: number) => {
          ...
          return (
            <div
              key={group.id}
              className="snap-start shrink-0 w-[44vw] sm:w-[300px] md:w-[320px] relative rounded-2xl overflow-hidden bg-card gold-card-frame-subtle transition-all duration-300 hover:scale-[1.02]"
            >
...
export default MoreGroupsSection;
