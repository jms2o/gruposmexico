...
import { Crown, Star, Music, ChevronLeft, ChevronRight, Flame } from "lucide-react";
...
const TopRankingSection = ({ groups, cityName, whatsappNum }: TopRankingSectionProps) => {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl sm:text-3xl font-display font-extrabold text-foreground">
              Más contratados{" "}
              <span className="text-gold italic">esta semana</span>
            </h2>
            <p className="text-muted-foreground font-body text-sm mt-0.5">
              Los favoritos de nuestros clientes en {cityName}
            </p>
          </div>
        </div>
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4">
                <Link to={`/grupo/${group.id}`}>
                  <h3 className="text-xl sm:text-2xl font-display font-extrabold text-white drop-shadow-lg mb-1">
                    {group.name}
                  </h3>
                </Link>
                ...
                <p className="text-white/80 font-body text-sm mb-3">
                  Desde{" "}
                  <span className="text-gold font-extrabold text-xl">{group.price}</span>
